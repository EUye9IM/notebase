...
### 配置与错误处理
- **配置文件**：`~/.config/notebase/config.toml`，定义模型端点、API 密钥、阈值参数、IPC 路径等。
- **数据存储**：SQLite 数据库文件位于 `~/.config/notebase/notebase.db`，存储笔记元数据和向量嵌入。
- **错误处理**：daemon 崩溃后自动重启，客户端重连机制，操作幂等性保证。
