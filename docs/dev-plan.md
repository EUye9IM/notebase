...
- 删除：确认弹窗 + toast 撤销 5s（建议 store 层 `deleteEntry` 返回可回滚快照，UI 不自己缓存条目）　— **已于 v1.1 推翻：见 §6.10（撤销条整体移除）**
...
| 删除一条记录 | 3 | `test/ui/search_test.dart`「长按 → 删除 → 确认（3 步），确认后即删且无撤销条」 |
...
| P3-4 | 录音期间开录的笔记本被删 → 丢录音且内部 id 进文案 | 目标本不存在时落到当前本 | `recording_test.dart`「录音期间开录的笔记本被删除…」 |
...
## 6.10 功能变更（移除删除后的提示条与撤销能力）

| 变更 | 说明 | 用例 |
|---|---|---|
| 删除条目不再有 toast + 5s 撤销 | 产品决策（v1.1，推翻 v1 的「撤销是唯一的后悔药」）：撤销要配套可回滚快照、归属回退、计时、以及 SnackBar 的 `persist` 坑，复杂度不低而收益很低；防误删由**删前确认**承担。删除成功不再打扰，只有**失败**才给可读提示（复检 P2 的可见性要求不变） | `search_test.dart`「长按 → 删除 → 确认（3 步），确认后即删且无撤销条」（同时断言没有「已删除」与「撤销」） |
| core 移除 `AppStore.restoreEntry`，`deleteEntry` 不再返回快照 | 撤销是这两个 API 的唯一消费者，能力移除后一并删掉，不留死代码（`Future<Entry>` → `Future<void>`） | `store_test.dart`「删除并持久化（v1.1 起不再返回可回滚快照）」；原撤销相关用例（复位 / 回到原本 / 落当前本 / 幂等）随能力一起删除，其中 §6.7 T-2 的用例也在内 |

保留的部分：
- 删除**仍要确认**（破坏性操作例外原则不变），§11 的「删除一条记录 = 3 步」不变。
- 条目删除**仍不动媒体文件**：回收统一交给「无主媒体清理」（§7）——理由从「撤销窗口内还要用」改成
  「避免在普通删除路径上做不可逆的文件删除」。
