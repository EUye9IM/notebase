...
- **整体复检登记项（尚未修，2026 复检报告）**：音频缺失未渲染为不可播放（§10）、
  照片缩略图无 `cacheWidth`（原图整幅解码）、转写兜底未截断（§4）、写盘失败无回滚与无提示、
  启动扫描异常逃逸（`startup.dart:39`）、`deleteEntries` 绕过写队列、多实例无数据目录锁、
  测试侧三处语义空洞（照片搜索命中面 / 撤销回原笔记本 / 退出搜索滚动位置用例偏弱）。
  逐条详见复检报告，按批次修。
